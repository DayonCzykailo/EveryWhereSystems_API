...
public interface AccontRepository extends JpaRepository<AccontModel, String> {
    ...
    @Query(value = "select * from tb_accont where tb_accont.id in (select tb_accont_id from tb_role_accont role where tb_role_id = (select tb_role.id from tb_role where tb_role.role = :role)) and tb_accont.empresa_id = :empresaId", nativeQuery = true)
